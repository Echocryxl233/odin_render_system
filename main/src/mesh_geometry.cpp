...
GeometryGenerator::MeshData GeometryGenerator::CreateSphere(float radius, uint32 sliceCount, uint32 stackCount)
{
  ...
  float phi_step = XM_PI / stackCount;
  float theta_step = 2.0f * XM_PI / sliceCount;
  ...
  uint32 base_index = 1;
  uint32 ring_vertex_count = sliceCount + 1;
  for (uint32 i = 0; i < stackCount - 2; ++i)
  {
    for (uint32 j = 0; j < sliceCount; ++j)
    {
      mesh_data.Indices32.push_back(base_index + i * ring_vertex_count + j);
      mesh_data.Indices32.push_back(base_index + i * ring_vertex_count + j + 1);
      mesh_data.Indices32.push_back(base_index + (i + 1) * ring_vertex_count + j);

      mesh_data.Indices32.push_back(base_index + (i + 1) * ring_vertex_count + j);
      mesh_data.Indices32.push_back(base_index + i * ring_vertex_count + j + 1);
      mesh_data.Indices32.push_back(base_index + (i + 1) * ring_vertex_count + j + 1);
    }
  }
  ...
  uint32 south_pole_index = (uint32)mesh_data.Vertices.size() - 1;
...
void GeometryGenerator::Subdivide(MeshData& meshData)
{
  ...
  uint32 numTris = (uint32)input_copy.Indices32.size() / 3;
    ...
    float radius, uint32 numSubdivisions)
{
  ...
  mesh_data.Indices32.assign(&k[0], &k[60]);
  ...
  for (uint32 i = 0; i < mesh_data.Vertices.size(); ++i)
  {
    ...
    mesh_data.Vertices[i].TexC.x = theta / XM_2PI;
    mesh_data.Vertices[i].TexC.y = phi / XM_PI;

    // Partial derivative of P with respect to theta
    mesh_data.Vertices[i].TangentU.x = -radius * sinf(phi) * sinf(theta);
    mesh_data.Vertices[i].TangentU.y = 0.0f;
    mesh_data.Vertices[i].TangentU.z = +radius * sinf(phi) * cosf(theta);

    XMVECTOR T = XMLoadFloat3(&mesh_data.Vertices[i].TangentU);
    XMStoreFloat3(&mesh_data.Vertices[i].TangentU, XMVector3Normalize(T));
  }
    ...
    uint32 sliceCount, uint32 stackCount)
{
  ...
  for (uint32 i = 0; i < stackCount; ++i)
  {
    for (uint32 j = 0; j < sliceCount; ++j)
    {
      mesh_data.Indices32.push_back(i * ring_vertex_count + j);
      mesh_data.Indices32.push_back((i + 1) * ring_vertex_count + j);
      mesh_data.Indices32.push_back((i + 1) * ring_vertex_count + j + 1);

      mesh_data.Indices32.push_back(i * ring_vertex_count + j);
      mesh_data.Indices32.push_back((i + 1) * ring_vertex_count + j + 1);
      mesh_data.Indices32.push_back(i * ring_vertex_count + j + 1);
    }
  }

  BuildCylinderTopCap(bottomRadius, topRadius, height, sliceCount, stackCount, mesh_data);
  BuildCylinderBottomCap(bottomRadius, topRadius, height, sliceCount, stackCount, mesh_data);
...
{
  uint32 base_index = (uint32)meshData.Vertices.size();
  ...
  uint32 center_index = (uint32)meshData.Vertices.size() - 1;

  for (uint32 i = 0; i < sliceCount; ++i)
  {
    meshData.Indices32.push_back(center_index);
    meshData.Indices32.push_back(base_index + i + 1);
    meshData.Indices32.push_back(base_index + i);
  }
}

void GeometryGenerator::BuildCylinderBottomCap(float bottomRadius, float topRadius, 
    float height, uint32 sliceCount, uint32 stackCount, MeshData& meshData)
{
  // 
  // Build bottom cap.
  //
...
GeometryGenerator::MeshData GeometryGenerator::CreateGrid(float width, float depth, uint32 m, uint32 n)
{
  ...
  for (uint32 i = 0; i < m; ++i)
  {
    ...
    for (uint32 j = 0; j < n; ++j)
    {
      ...
      mesh_data.Vertices[i * n + j].TexC.x = j * du;
      mesh_data.Vertices[i * n + j].TexC.y = i * dv;
    }
  }
...
//
  ...
  mesh_data.Indices32.resize(face_count * 3); // 3 indices per face
