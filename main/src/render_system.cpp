...
void RenderSystem::Initialize() {
  ...
  forward_shading_->SetRenderQueue(&Graphics::MainRenderQueue);

  deferred_shading_ = new Graphics::DeferredShading();
  deferred_shading_->Initialize();
  deferred_shading_->SetRenderQueue(&Graphics::MainRenderQueue);
...
void RenderSystem::DrawDebug(GraphicsContext& context) {
  ...
  D3D12_CPU_DESCRIPTOR_HANDLE handles2[] = { Graphics::Core.DepthBuffer().DepthSRV() };
